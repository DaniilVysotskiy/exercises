...
minmax x y z = maxN x y z - minN x y z
    where
        maxN :: Integer -> Integer -> Integer -> Integer
        maxN a b c = maximum [a, b, c]
        minN :: Integer -> Integer -> Integer -> Integer
        minN a b c = minimum [a, b, c]
...
subString start end str = do
    if (end < 0)
        then ""
    else do
        if (start < 0)
            then take (end + 1) (drop 0 str)
        else take ((end + 1) - start) (drop start str)
...
strSum str = do
    let numList = map read (words str)
    if null numList
        then 0
    else last (scanl1 (+) numList)
...
lowerAndGreater n list = show n ++ " is greater than " ++ show (greater 0 list) ++ " elements and lower than " ++ show (lower 0 list) ++ " elements"
    where
        ...
        greater result l =
            ...
            then result
                ...
                then greater (result + 1) (tail l)
                else greater result       (tail l)
        lower ::  Integer -> [Integer] -> Integer
        lower result l =
            if null l
            then result
            else if head l > n
                then lower (result + 1) (tail l)
                else lower result       (tail l)
